...
for page in range(1, 163):
    list_url = f"https://search.ltn.com.tw/list?keyword=%E6%A6%AE%E7%B8%BD&start_time=20220101&end_time=20250629&sort=date&type=all&page={page}"
    list_driver.get(list_url)
    news_list = list_driver.find_elements(By.CSS_SELECTOR, "div.cont a[class='tit']")
    ...
    with open(content_path, "a", encoding="utf-8") as f:
        for i, element in enumerate(news_list):
            ...
            try:
                ...
                date = driver.find_element(By.CSS_SELECTOR, "h1~div .time").text
                ...
                while loc_y < body_height:
                    ...
                    loc_y += 500
                    time.sleep(0.04)
                contents = driver.find_elements(By.CSS_SELECTOR, "div[data-desc='內文'] p:not([style], [class]), div[data-desc='內容頁'] p:not([style], [class])")
                f.write("--------------------------\n")
                f.write(date + "\n")
                f.write(title + "\n")
                for p in contents:
